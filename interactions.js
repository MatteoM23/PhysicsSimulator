...
function createAuroraEffect(bodyA, bodyB, engine) {
    ...
    if (engine.render && engine.render.canvas && !auroraEffectApplied) {
        Matter.Events.on(engine, 'beforeRender', () => {
            drawAuroraAnimation(engine.render.canvas, engine.render.context);
        });
        auroraEffectApplied = true; // Prevent multiple event attachments
    } else if (!engine.render || !engine.render.canvas) {
        console.error("Canvas context not found. Ensure you are using Matter.js's built-in renderer or adjust accordingly.");
    }
}
...
function drawAuroraAnimation(canvas, ctx) {
    ...
    // Create a gradient effect for the aurora
    ...
    // Set the global composite operation to 'destination-over' to draw behind the current content
